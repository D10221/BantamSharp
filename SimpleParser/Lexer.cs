...
namespace SimpleParser
{
    ...
    public class Lexer<T> : ILexer
    {
        ...
        /// </summary>
        public Lexer(IEnumerable<T> source)
        {
            mIndex = 0;
            _source = source;//.ToArray();//.Where(c => !Char.IsWhiteSpace(c)).ToArray();
            _enumerator = source.GetEnumerator();//.ToCharArray().Cast<Char>().GetEnumerator();
        }

        private IToken TryGetPunctuator(T c)
        {
        ...
        public string InputText
        {
            get
            {
                return _source.Select(a => a.ToString(CultureInfo.InvariantCulture)).Aggregate((a, b) => a + b);
            }
        }
